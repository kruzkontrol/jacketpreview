...
(function () {
  ...
  function writeMockupLink(values) {
    ...
    // Find the internal field by its label and write the URL into it.
    var els = document.querySelectorAll("input[type=text], textarea");
    for (var i = 0; i < els.length; i++) {
      var lbl = "";
      try { lbl = (labelFor(els[i]) || []).join(" "); } catch (e) {}
      if (norm(lbl).indexOf("INTERNAL DO NOT EDIT") !== -1
          || norm(lbl).indexOf("INTERNAL") === 0) {
        if (els[i].value !== url) {
          els[i].value = url;
