...
(function () {
  ...
  function frame() {
    ...
    try {
      ...
      f.style.cssText = "width:100%;height:" + PREVIEW_HEIGHT + ";border:0";
      host.appendChild(f);

      // Put it after the product's option form if we can find it,
      // otherwise at the end of the product details.
      // Place the preview right AFTER the product's option fields, so
      // it sits with the choices - not dumped at the bottom under the
      // reviews and footer. Find the last option field and insert after
      // its containing block; fall back to the details area, then body.
      var placed = false;
      try {
        var opts = document.querySelectorAll(
          ".ec-store select, .ec-store input, .product-details select, " +
          "[class*='product'] select, select, input");
        // the last relevant option element on the page
        var last = null;
        for (var oi = 0; oi < opts.length; oi++) {
          var el = opts[oi];
          // skip our own and obvious non-option controls
          if (el.type === "hidden" || el.type === "submit") continue;
          last = el;
        }
        if (last) {
          // walk up to a block-level container to insert after
          var block = last;
          for (var up = 0; up < 4 && block.parentNode; up++) {
  ...
  function send() {
    ...
    try {
      ...
      tag.textContent = "preview v9 - jacket: " +
    ...
    try {
      ...
      if (f.getAttribute("data-kwp-src") !== newSrc) {
        ...
        repl.style.cssText = f.style.cssText ||
          ("width:100%;height:" + PREVIEW_HEIGHT + ";border:0");
  ...
  if (window.Ecwid && Ecwid.OnPageLoaded) {
    Ecwid.OnPageLoaded.add(function (page) {
      // ONLY on a product page. Ecwid fires this for every page - cart,
      // category, account - and building the preview on all of them is
      // why it showed everywhere. On any other page, clear it.
