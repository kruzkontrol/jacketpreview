...
(function () {
  ...
  function writeMockupLink(values) {
    ...
    for (var i = 0; i < els.length; i++) {
      var el = els[i];
      // Skip anything the bridge already knows as an option.
      var known = null;
      ...
      var nearest = norm(chain[0] || "");
      if (nearest.indexOf("INTERNAL") === 0
          || nearest.indexOf("INTERNAL DO NOT EDIT") !== -1) {
        if (el.value !== url) {
          el.value = url;
          el.dispatchEvent(new Event("input", { bubbles: true }));
          el.dispatchEvent(new Event("change", { bubbles: true }));
