...
(function () {
  ...
  // How tall the preview frame is on the product page. Change this one
  // number if it looks too big or too small.
  ...
  var FIELDS = {
    ...
    "LINE NUMBER/CHAPTER GREEK LETTERS": "num_or_chapter",
    "LINE NUMBER CHAPTER GREEK LETTERS": "num_or_chapter",
    "CHAPTER": "chapter",
    "OTHER ORG": "other_org",
    // The chapter's Greek-vs-normal switch. GREEK LETTERS draws it in
    // Greek; NORMAL TEXT spells it out in block letters.
    "NON GREEK EX ZETA EX LMAC": "chapter_style",
    "NON GREEK": "chapter_style",
    "CROSSING SEASON": "crossing",
    "NECK LETTERING": "neck",
    "JACKET COLOR": "jacket",
    // The colour option can be named to suit the garment - all map to
    // the same field. The VALUE (e.g. "DUFFLE ROYAL", "TEE BLACK")
    // still tells the preview which garment it is; these are just the
  ...
  function orgCode(raw) {
    ...
    // The "OTHER (TYPE BELOW)" pick is a real choice - the customer
    // types their letters in the Other org. box. Send OTHER so the
    // preview knows to draw the typed custom letters.
    if (code === "OTHER" || code === "OTHERTYPEBELOW") return "OTHER";
  ...
  function fieldFor(el) {
    ...
    // to the pocket-wording field and was mis-mapped to it, so its
    // value ("GREEK LETTERS") landed on the pocket.
    if (el.type === "radio") {
      var own = String(el.value || "");
      try {
        var wrap = el.closest ? el.closest("label") : null;
        if (wrap) own += " " + (wrap.textContent || "");
        else if (el.parentElement) own += " " + (el.parentElement.textContent || "");
      } catch (e) {}
      var o = norm(own);
      if (o.indexOf("GREEK LETTERS") !== -1
          || o.indexOf("NORMAL TEXT") !== -1) {
  ...
  function collect() {
    ...
    for (var i = 0; i < els.length; i++) {
      ...
      if (COLOUR_FIELDS[field]) {
        ...
        values[field] = orgCode(raw);
                 ...
                 || field === "crest_on" || field === "stole_on") {
        // The store's value carries its price - "YES (+$25.00)".
        values[field] = (norm(raw).indexOf("YES") === 0) ? "YES" : "NO";

      } else if (field === "num_or_chapter") {
        // ONE text field on the bag, shared between the line number and
        // the chapter. The Line Number / Greek Letters / Normal Text
        // radio decides which. Read the CHECKED radio to route the text.
        var typed = String(raw).trim();
        var mode = "";
        try {
          var radios = document.querySelectorAll("input[type=radio]");
          for (var r = 0; r < radios.length; r++) {
            if (!radios[r].checked) continue;
            var rown = String(radios[r].value || "");
            var rw = radios[r].closest ? radios[r].closest("label") : null;
            if (rw) rown += " " + (rw.textContent || "");
            else if (radios[r].parentElement)
              rown += " " + (radios[r].parentElement.textContent || "");
            var rn = norm(rown);
            if (rn.indexOf("LINE NUMBER") !== -1
                || rn.indexOf("GREEK LETTERS") !== -1
                || rn.indexOf("NORMAL TEXT") !== -1) {
              mode = rn; break;
            }
          }
        } catch (e) {}

        if (mode.indexOf("LINE NUMBER") !== -1) {
          values.number = typed;              // it's a number
        } else if (mode.indexOf("NORMAL") !== -1) {
          values.chapter = typed;             // chapter, spelled out
          values.chapter_style = "NORMAL";
        } else {
          values.chapter = typed;             // chapter, Greek (default)
          values.chapter_style = "GREEK";
        }

      } else if (field === "chapter_style") {
        // Radio: GREEK LETTERS vs NORMAL TEXT. Read ONLY the chosen
        // radio's own value and its immediate label - never the group,
        // which contains both words. Default to GREEK.
        var own = String(el.value || "");
        try {
          var wrap = el.closest ? el.closest("label") : null;
          if (wrap) own += " " + (wrap.textContent || "");
            ...
            if (lf) own += " " + (lf.textContent || "");
          }
          if (el.parentElement) own += " " + (el.parentElement.textContent || "");
        } catch (e) {}
        values[field] = (norm(own).indexOf("NORMAL") !== -1)
          ? "NORMAL" : "GREEK";
    ...
    // ---- STOLE boxes, read BY POSITION -----------------------------
    // Only runs when the chosen colour is a STOLE. The four wording
    // boxes are the only text inputs on a stole product and appear in
    // this order top-to-bottom: 0=TOP LEFT, 1=BOTTOM LEFT,
    // 2=TOP RIGHT, 3=BOTTOM RIGHT. We read them by that order rather
    // than by label, because the label matching does not reach them.
    // A jacket never enters this block - its colour is not a STOLE.
    if (String(values.jacket || "").toUpperCase().indexOf("STOLE") === 0) {
      var tboxes = [];
      var allText = document.querySelectorAll("textarea, input[type=text]");
      for (var t = 0; t < allText.length; t++) {
        var tb = allText[t];
        if (tb.name && /qty|quantity/i.test(tb.name)) continue;
        tboxes.push(tb);
      }
      if (tboxes[0]) values.stole_tl = tboxes[0].value;
      if (tboxes[1]) values.stole_bl = tboxes[1].value;
      if (tboxes[2]) values.stole_tr = tboxes[2].value;
      if (tboxes[3]) values.stole_br = tboxes[3].value;
    }
  ...
  function send() {
    ...
    var values = collect();
    if (typeof centreImage === "string") {
      values.centre_image = centreImage;   // "" clears it, a data-URL sets it
    }
    ...
    // Send the values to the pasted iframe by postMessage ONLY. This
    // never touches the iframe's src, so it never reloads and never
    // flashes - the picture updates in place. (An earlier version set
    ...
    // Write the recreate-URL into the "Internal - do not edit" field so
    // it rides to the order. Clicking it later redraws this exact
    // mockup. Nothing is saved - it is just a link built from the same
    // values the preview uses.
    try {
      writeMockupLink(values);
    } catch (e) {
      if (KWP_DEBUG) console.log("[kanework] mockup link failed", e);
    }
  }

  // Build the mockup recreate-URL and put it in the internal field.
  function writeMockupLink(values) {
    var qs = [];
    for (var k in values) {
    ...
    var url = "https://" + PREVIEW_HOST + "/"
              + (qs.length ? "?" + qs.join("&") : "");

    // Find the internal field by its OWN nearest label and write the
    // URL into it. Match only the nearest label (not the ancestor
    // chain, which can pick up a neighbour's "Internal" text), and
    // never write into a field that maps to a real option (jacket,
    // letter, ship, the stole boxes, etc.).
    var els = document.querySelectorAll("input[type=text], textarea");
    for (var i = 0; i < els.length; i++) {
      var el = els[i];
      // Skip anything the bridge knows as an option.
      var known = null;
      try { known = fieldFor(el); } catch (e) {}
      if (known) continue;

      var chain = [];
      try { chain = labelFor(el) || []; } catch (e) {}

      // Find where "INTERNAL" and "OTHER ORG" each first appear in the
      // label chain. The field is the Internal one only if INTERNAL is
      // CLOSER (a lower index) than any other known label - otherwise
      // we are seeing a neighbour's Internal label leaking in, and this
      // is really the Other org box or something else.
      var iInternal = -1, iOther = -1;
      for (var c = 0; c < chain.length; c++) {
        var t = String(chain[c]);
        if (iInternal < 0 && t.indexOf("INTERNAL") !== -1) iInternal = c;
        if (iOther < 0 && t.indexOf("OTHER ORG") !== -1) iOther = c;
      }
      if (iInternal < 0) continue;                 // no internal label
      if (iOther >= 0 && iOther <= iInternal) continue;  // it's Other org
  ...
  // The uploaded centre image, as a data-URL. Read whenever a file
  // input changes; a redraw then sends it to the preview.
  var centreImage = "";
  document.addEventListener("change", function (ev) {
    var el = ev.target;
    if (!el || el.type !== "file") return;

    // ONLY the "IMAGE" upload feeds the mockup - not the LOGO uploads.
    // The IMAGE file input's aria-label starts with "IMAGE"; the logo
    // ones are "LOGO 1", "LOGO 2", etc. Match on that so a logo upload
    // never lands in the number spot.
    var aria = (el.getAttribute && el.getAttribute("aria-label")) || "";
    if (norm(aria).indexOf("IMAGE") !== 0) return;   // not the IMAGE box

    var file = el.files && el.files[0];
    if (!file) { centreImage = ""; schedule(); return; }
    if (!/^image\//.test(file.type)) return;   // only images
    var reader = new FileReader();
    reader.onload = function (e) {
      centreImage = String(e.target.result || "");
      schedule();
    };
  ...
  // Remove any preview we built - used when leaving a product page, so
  // the mockup does not linger on the cart, categories, or every other
  // storefront page.
  function removePreview() {
    try {
      var f = document.querySelector('iframe[data-kwp="1"]');
      if (f) {
        // remove the wrapper we created (title + iframe), else the frame
        var box = f.parentNode;
        (box && box.parentNode ? box.parentNode : document.body)
          .removeChild(box || f);
      }
    } catch (e) {}
  }
  ...
  if (window.Ecwid && Ecwid.OnPageLoaded) {
    Ecwid.OnPageLoaded.add(function () {
      // Two goes, on their own timers. The pasted iframe only exists on
      // the products where you added it, so on other pages frame()
      // returns null and send() simply does nothing.
      setTimeout(send, 800);
