...
(function () {
  ...
  function send() {
    ...
    try {
      ...
      tag.textContent = "preview v10 - jacket: " +
    ...
    // draw. Only when the values changed, so it does not reload on
    // every message.
    try {
      var qs = [];
      for (var k in values) {
      ...
      var newSrc = "https://" + PREVIEW_HOST + "/" +
                   (qs.length ? "?" + qs.join("&") : "");
      if (f.getAttribute("data-kwp-src") !== newSrc) {
        f.setAttribute("data-kwp-src", newSrc);
        f.src = newSrc;
      }
    } catch (e) {
    ...
    // postMessage too, harmless, for any iframe that acts on it.
    try {
      if (f.contentWindow) {
