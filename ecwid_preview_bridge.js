...
(function () {
  ...
  function send() {
    ...
    try {
      ...
      tag.textContent = "preview v11 - jacket: " +
    ...
    //     every message.
    // Feed the pasted iframe by updating its URL - the path proven to
    // FIRST time only: set the iframe src with the values, so it draws
    // straight away (no blank). After that, updates go by postMessage
    // only - no reload, no flash. This keeps the smooth in-place update
    // while still guaranteeing the very first picture appears.
    try {
      if (!f.getAttribute("data-kwp-loaded")) {
        var qs = [];
        for (var k in values) {
        ...
        var firstSrc = "https://" + PREVIEW_HOST + "/" +
                       (qs.length ? "?" + qs.join("&") : "");
        f.setAttribute("data-kwp-loaded", "1");
        f.src = firstSrc;
      }
    } catch (e) {
      if (KWP_DEBUG) console.log("[kanework] first load failed", e);
    }

    // Every update - including changes after the first load - goes by
    // postMessage, which redraws inside the frame WITHOUT reloading it.
    // No flash.
    try {
      if (f.contentWindow) {
