...
(function () {
  ...
  // The nine organisations, as the artwork folder names them: the
  // store's wording with the spaces taken out. Anything else - OTHER,
  // or nothing chosen - draws no Greek and no crest.
  var ORGS = {
    "ALPHA PHI ALPHA": "ALPHAPHIALPHA",
    "KAPPA ALPHA PSI": "KAPPAALPHAPSI",
    "OMEGA PSI PHI": "OMEGAPSIPHI",
    "IOTA PHI THETA": "IOTAPHITHETA",
    "ALPHA KAPPA ALPHA": "ALPHAKAPPAALPHA",
    "DELTA SIGMA THETA": "DELTASIGMATHETA",
    "ZETA PHI BETA": "ZETAPHIBETA",
    "SIGMA GAMMA RHO": "SIGMAGAMMARHO"
  };
  ...
  // The two the preview can recommend a colour for.
  ...
  function recommended(org, jacket) {
    ...
    if (byJacket && byJacket[jacket]) return byJacket[jacket];
  ...
  function labelFor(el) {
    if (el.id) {
      ...
      if (byFor) return [norm(textWithoutControls(byFor))];
    }
  ...
  function collect() {
    ...
    for (var i = 0; i < els.length; i++) {
      ...
      if (COLOUR_FIELDS[field]) {
        ...
        // Nothing chosen for the LETTERING means "use what this
        // organisation usually sews on this jacket" - AUTO - rather
        // than holding whatever was showing before. The number's own
        // colours have no such recommendation, so they still hold.
        ...
        values[field] = ORGS[norm(raw)] || "NONE";

      } else if (field === "wording") {
        // The store's value carries its price - "YES (+$10.00)".
