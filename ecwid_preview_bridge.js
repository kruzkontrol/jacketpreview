...
(function () {
  ...
  function collect() {
    ...
    for (var i = 0; i < els.length; i++) {
      ...
      if (COLOUR_FIELDS[field]) {
        ...
        // YES/NO. Ecwid often sets a radio's .value to an internal id,
        // not the word "Yes" - so check the visible LABEL text of the
        // chosen radio as well as its value. Either saying YES counts.
        var lbl = "";
        try { lbl = (labelFor(el) || []).join(" "); } catch (e) {}
        var hay = norm(String(raw) + " " + lbl);
        values[field] = (hay.indexOf("YES") !== -1) ? "YES" : "NO";
  ...
  function send() {
    ...
    try {
      ...
      tag.textContent = "preview v16 - jacket: " +
