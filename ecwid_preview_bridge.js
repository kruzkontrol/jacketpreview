...
(function () {
  ...
  var FIELDS = {
    ...
    "ADD CREST": "crest_on",
    "CREST": "crest_on",
    // Stole - the four wording areas on the legs, and the customize
    // Yes/No. These feed the stole preview.
    "CUSTOMIZE": "stole_on",
    "TOP LEFT": "stole_tl",
  ...
  function labelFor(el) {
    ...
    while (node && hops < 8) {
      ...
      // Ecwid puts the option name ("TOP LEFT") in a HEADING that is a
      // PRECEDING SIBLING of the input's block, not an ancestor of the
      // input. So at each level, also read the previous siblings - that
      // is where the real label lives for a text box.
      var sib = node.previousElementSibling;
      var sibHops = 0;
      while (sib && sibHops < 3) {
        var st = norm(textWithoutControls(sib));
        if (st) out.push(st);
  ...
  function collect() {
    ...
    for (var i = 0; i < els.length; i++) {
      ...
      if (COLOUR_FIELDS[field]) {
        ...
        // YES/NO. The console proved Ecwid sets the radio VALUE to the
        // literal "YES"/"NO" (and the label is empty). collect() only
        // reaches the CHECKED radio - the unchecked one returns null
        // from valueOf and is skipped - so raw here IS the chosen
        // value. Just read it. (Earlier versions read the whole group's
        // text and always found "Yes"; that was the bug.)
        var yn = norm(raw);
        values[field] = (yn.indexOf("YES") === 0) ? "YES" : "NO";
  ...
  function send() {
    ...
    try {
      ...
      tag.textContent = "preview v18 - jacket: " +
