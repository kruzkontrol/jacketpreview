...
(function () {
  ...
  function writeMockupLink(values) {
    ...
    for (var i = 0; i < els.length; i++) {
      var el = els[i];
      // Skip anything the bridge already knows as an option (jacket,
      // letter, ship, the stole boxes, etc.) - the URL must never land
      // in one of those.
      var known = null;
      try { known = fieldFor(el); } catch (e) {}
      if (known) continue;

      // The Internal label may sit a level or two up (Ecwid puts the
      // option name in a heading above the box), so check the label
      // chain, not just the nearest. Safe because known-option fields
      // are already skipped above.
      var chain = [];
      try { chain = labelFor(el) || []; } catch (e) {}
      var hit = false;
      for (var c = 0; c < chain.length; c++) {
        if (String(chain[c]).indexOf("INTERNAL") !== -1) { hit = true; break; }
      }
      if (hit) {
        if (el.value !== url) {
          el.value = url;
