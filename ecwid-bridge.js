...
(function () {
  ...
  // ---------- CONFIG ----------
  // Activate the live preview on the test product. We match by Ecwid PRODUCT ID
  // (most reliable — never changes), with a URL-fragment fallback. The product
  // ID for "PLATINUM LINE JACKET test" is 837474642.
  var TEST_PRODUCT_ID = 837474642;
  // Fallback: also activate if the URL contains this fragment (case-insensitive).
  ...
  // ---------- GUARDS ----------
  // We're on the target product page if EITHER:
  //  (a) Ecwid's storefront API reports the current page is our product ID, OR
  //  (b) the URL contains the test fragment (fallback for odd URL setups).
  var currentEcwidProductId = null;  // set by Ecwid.OnPageLoaded below

  function isTargetProductPage() {
    if (currentEcwidProductId && Number(currentEcwidProductId) === TEST_PRODUCT_ID) {
      return true;
    }
  ...
  // Hook into Ecwid's storefront JS API to reliably detect the product page.
  // Ecwid fires OnPageLoaded with page.type === 'PRODUCT' and page.productId.
  // This is more reliable than URL sniffing across Ecwid's routing modes.
  if (typeof window.Ecwid !== 'undefined' && Ecwid.OnPageLoaded && Ecwid.OnPageLoaded.add) {
    Ecwid.OnPageLoaded.add(function (page) {
      if (page && page.type === 'PRODUCT') {
        currentEcwidProductId = page.productId;
      } else {
        currentEcwidProductId = null;
      }
      // Re-run init now that we know the page type
      initWhenReady();
    });
  }
